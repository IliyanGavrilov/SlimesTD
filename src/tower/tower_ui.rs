...
fn update_tower_ui(
  ...
  mut tower_upgrade_cost_ui: Query<(&mut Text, &TowerUpgradeCost),
    (Without<TowerStatsUI>, Without<TowerLifetimeStatsUI>,
      Without<TargetingPriorityUI>, Without<SellButtonText>)>,
  mut tower_upgrade_index_image_ui: Query<(&mut UiImage, &TowerUpgradeIndex)>,
  mut upgrade_stats: Query<(&mut Text, &UpgradeStats),
    (Without<TowerStatsUI>, Without<TowerLifetimeStatsUI>,
     Without<TargetingPriorityUI>, Without<SellButtonText>, Without<TowerUpgradeCost>)>
) {
  ...
  for parent in child_q.iter_mut() {
    ...
    for (mut upgrade_cost_text,
      tower_upgrade_cost) in tower_upgrade_cost_ui.iter_mut() {
      let i = tower.upgrades.upgrades[tower_upgrade_cost.path_index];
      let tower_upgrades =
        &upgrades.upgrades[tower_type][tower_upgrade_cost.path_index];
    ...
    for (mut upgrade_index_image,
      tower_upgrade_index) in tower_upgrade_index_image_ui.iter_mut() {
      *upgrade_index_image = assets.upgrades
        [tower.upgrades.upgrades[tower_upgrade_index.path_index]].clone().into();
    }
    ...
    for (mut stats_text, upgrade_stats) in upgrade_stats.iter_mut() {
      ...
      if i < tower_upgrades.len() {
        ...
        *stats_text = Text::from_section(upgrade_stats_string,
                                         stats_text.sections[0].style.clone());
      }
      else {
        *stats_text = Text::from_section("",
                                         stats_text.sections[0].style.clone());
      }
    }
  }
